...
void segundapassagem (list<simb> tab_simb){
  ...
  int i, tam_rot, parametro, num_op, contadorpos=0, contadorlinha=0, mudaop=0, num_dir=0;
  simb elemento_simb;
      ...
      if ((num_op>=1 && num_op<=8)||(num_op>=10 && num_op<=13)){
        ...
        if (iterador == tab_simb.end()){
          ...
          if (iterador2 == tab_simb.end()){
            ...
            printf("operando2:%d ", iterador2->endereco);
            fputs(" ", instrucoes2);
          }
        }
